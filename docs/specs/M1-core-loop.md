...
| 항목 | 내용 |
|---|---|
| 상태 | **Verified** (2026-07-08 — 테스트 93 + Playwright 스테이지 클리어 검증) |
| 작성일 | 2026-07-08 |
...
- [x] 채점 유닛테스트 전부 통과(완벽 원·반토막·구멍·융합 — GDD §6.4)
- [x] 뒤집기 판정이 게이지 조작으로 재현(Playwright: 클린·이탈; 반접힘/발사/까만뒷면은 판정표+코드)
- [x] 하드코딩 스테이지 1개를 처음부터 끝까지 **클리어 가능**(대기열 소진 → STAGE CLEAR)
- [x] 재고 < 남은 주문 시 실패 판정 동작(StageSession 단위 테스트)
- [x] `npm run test` 그린(93) + Playwright 시각 QA + 콘솔 에러 0
...
**순수 로직 코어 (§7-1~5 + StageSession):** `geometry`·`scoring`(GDD §6.4 필수 케이스: 완벽 원·반토막·구멍·융합)·`flip`(게이지+판정표)·`inventory`·`orders`·`stage`(세션 상태기계) + balance(FLIP/SCORE/ORDER/STAGE1). **Vitest 50개 추가**(총 93 그린).

**상호작용·뷰 (§7-7~12):** 제스처 상태기계(짧은 탭=깨기[주문 수 캡+재고 차감] / 홀드-릴리즈=뒤집기[게이지] / 위로 스와이프=서빙[원형도 채점]) · 뒤집기 애니(포물선·착지 스쿼시·반접힘 폴리곤 접기·이탈/발사 날아감) · 파워 게이지 뷰 · 점수 팝업 · Bacon 톤 손님 큐 + 말풍선 · 재고 HUD · 결과 화면(STAGE CLEAR / FAILED).

**스테이지 진행 (StageSession):** 크랙→익힘→뒤집기→서빙→다음 손님, 전원 서빙=클리어. 까만뒷면=주문 실패(재고 도난), 재고<남은 주문=데드락 실패, 스모크 방치=즉시 실패. 하드코딩 스테이지 1(손님 5·주문 1~2·여유 3).

**Playwright 시각 검증 (콘솔 에러 0):**
- `qa/verify-m1-flip.mjs`: 게이지 바(스윗스팟/마커) · 클린 착지 · 오버차지 이탈(FLEW_OFF) · 스와이프 서빙 점수(99.693, 100 미도달).
- `qa/verify-m1-stage.mjs`: 손님 5명 전원 처리 → **STAGE CLEAR** (avg 99.534, served 5, failed 0). 큐 진행·재고 감소·점수 팝업 확인.

**as-built 차이:**
- 게이지는 홀드가 탭 임계(`TAP_MAX_MS`=180ms)를 넘을 때만 표시 — 빠른 탭(깨기)과 홀드(뒤집기)를 한 포인터로 구분.
- 발사체/이탈은 "위로 날아가 소실"까지(손님 피격·아이템 도난 연쇄는 M4). 반접힘은 폴리곤을 가로로 접어 원형도(점수)를 실제로 낮춘다.
- 결과 화면 요약(avg/served/failed)만 M1 — 후라이 배열·카운트업·별점·PNG 공유는 M3.
