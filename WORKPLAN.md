...
깨기→익힘→뒤집기→채점→서빙의 한 판을 하드코딩 스테이지 1개로 완성한다. 원형도 채점이 이 게임 점수의 심장이다.

- [ ] 왕복 파워 게이지(홀드 0→1→0) + 뒤집기 판정 전부 — 발사(RAW)/반접힘/이탈/클린 (GDD §6.3)
- [ ] 팬 틸트 + 계란 포물선 + 착지 스쿼시 애니
- [ ] 원형도 채점 Q = 4πA/P² + 유닛테스트(완벽 원/반토막/구멍 1~2개/융합) (GDD §6.4)
- [ ] 스와이프 서빙 — 방식은 미결 [DECISION-03], 확정 후 구현
- [ ] 손님 큐(3~4명)/말풍선 주문/계란 재고 HUD + 재고 부족 판정(GDD §6.5)
- [ ] 하드코딩 스테이지 1개 처음부터 끝까지 클리어 가능
