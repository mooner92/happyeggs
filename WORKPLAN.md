...
## M2 — 이벤트 프레임워크 (스펙: Verified — 2026-07-08)

"JSON 항목 + 핸들러 1개 등록"만으로 새 적을 추가하는 데이터 주도 이벤트 시스템(GDD §8)을 세우고, 적 2종으로 증명했다. 상세: [docs/specs/M2-event-framework.md](docs/specs/M2-event-framework.md).

- [x] 이벤트 스케줄러 — 조리 중에만 발생, `eventBudget`/`cooldownMs`/`maxConcurrent`/`stageUnlock`, 시드 결정론
- [x] 공통 telegraph → responseWindow → resolve 파이프라인 (GDD §8 공통 스키마)
- [x] 닌자 거미 — 드래그 절단 성공(낙하 + 거미줄 트로피) / 방치 실패(후라이 반토막)
- [x] 뒷문 강도 — 더블탭 고양이 성공 / 방치 실패(노른자 파손)
- [~] 오브젝트 풀링 — maxConcurrent 1이라 create/destroy로 충분, 대량 동시는 M6 성능 패스
...
## M5 — 야간 + 스킨 (스펙: Verified — 2026-07-09)

야간 스테이지의 열화상 컨셉(GDD §8.1 ⑦)과 계란 스킨 BM 스캐폴드(GDD §12)를 붙인다.

- [x] 열화상 표현 — 남색 오버레이 + 뜨거운 것(불꽃·조리 계란)만 hot depth로 밝게 (셰이더 대신 depth 레이어링)
- [x] 불 끄기 적 fire_snuffer(탭 저지·스토브 탭 재점화) + 가짜불 스티커(화롯불 주황 ↔ 차가운 파랑 대비)
...
## M6 — 폴리시 (스펙: Implemented — 폰 실측 대기)

게임필(연출)과 성능을 마감한다. v1 웹 출시 직전 상태.

- [ ] 이징/스쿼시&스트레치/파티클/화면 흔들림 폴리시 패스
- [x] SFX — 절차적 WebAudio 합성 19키 + 전 게임플레이/UI 훅 (에셋 0, `window.__sfxLog` QA) 연결(GDD §14 리스트 — 지글지글 피치 상승 포함, 현재 무음)
